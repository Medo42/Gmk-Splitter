...
/*
 * GmkSplitter modification notice
 *
 * This file overrides the LateralGM version bundled with the GmkSplitter v0.18
 * release (see build-release.sh). Changes relative to that version:
 *
 * - Added readGmFile(GmStreamDecoder, URI, ResNode, Charset): takes a
 *   caller-supplied decoder (so GmkSplitter can plug in a subclass that reports
 *   decoding problems) and the charset to use for the text of pre-8.1 (.gmk)
 *   files. 8.1 files are always decoded as UTF-8, as before.
 *
 *   Background: pre-8.1 text used to be decoded with Charset.defaultCharset(),
 *   which since Java 18 (JEP 400) is UTF-8 instead of the system code page that
 *   Game Maker actually used. The existing overloads keep their behaviour.
 *
...
public final class GmFileReader
	...
	public static GmFile readGmFile(InputStream stream, URI uri, ResNode root, Charset forceCharset)
			throws GmFormatException
		{
		return readGmFile(new GmStreamDecoder(stream),uri,root,forceCharset,null);
		}

	/**
	 * GmkSplitter addition. Reads a game file from the given decoder.
	 *
	 * @param gmkCharset the charset used for text in pre-8.1 (.gmk) files. If null,
	 *        Charset.defaultCharset() is used like in the other overloads. Ignored for
	 *        8.1 files, which are always UTF-8.
	 */
	public static GmFile readGmFile(GmStreamDecoder in, URI uri, ResNode root, Charset gmkCharset)
			throws GmFormatException
		{
		return readGmFile(in,uri,root,null,gmkCharset);
		}

	private static GmFile readGmFile(GmStreamDecoder in, URI uri, ResNode root,
			Charset forceCharset, Charset gmkCharset) throws GmFormatException
		{
		GmFile f = new GmFile();
		f.uri = uri;
		...
		try
			...
			else if (gmkCharset != null)
				in.setCharset(gmkCharset);
			else
